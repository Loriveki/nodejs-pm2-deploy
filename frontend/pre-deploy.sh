...
ENV_FILE=${LOCAL_ENV_PATH:-/home/loriveki/nodejs-pm2-deploy/frontend/.env}
...
if [ -f "$ENV_FILE" ]; then
  ...
  scp -i "$DEPLOY_SSH_KEY" "$ENV_FILE" "$DEPLOY_USER@$DEPLOY_HOST:/home/user/shared/frontend.env" >> /home/loriveki/pre-deploy-frontend-error.log 2>&1 || { echo "Failed to copy .env file" >> /home/loriveki/pre-deploy-frontend-error.log; exit 1; }
